...
const AboutUs = () => {
    ...
    const [isVideoEnded, setIsVideoEnded] = useState(false); // New state to track intro video
    ...
    /* 🎥 VIDEO VISIBILITY (Existing Scroll Logic) */
    ...
    return (
        <div ref={containerRef} className="relative h-[200vh] bg-slate-900">
            <section className="sticky top-0 w-full h-screen overflow-hidden">
                ...
                {/* 🎬 INTRO BACKGROUND VIDEO - Plays Once */}
                <video
                    ...
                    {isVideoEnded && (
                        ...
                        >
                            ...
                            <motion.div
                                ...
                                className="absolute top-[5%] left-[5%] z-50 cursor-pointer pointer-events-auto"
                            >
                                <div className="w-[3.5vw] h-[3.5vw] border-2 border-white/30 rounded-full flex flex-col items-center justify-center gap-[0.4vw] bg-black/30 backdrop-blur-md">
                                    <div className="w-[1.6vw] h-[2px] bg-white"></div>
                                    <div className="w-[1.6vw] h-[2px] bg-white"></div>
                                    <div className="w-[1.6vw] h-[2px] bg-white"></div>
                                </div>
                            </motion.div>
                            ...
                            <motion.div
                                ...
                                className="absolute top-[6%] left-1/2 -translate-x-1/2 z-50 flex items-center gap-[3vw] pointer-events-auto"
                            >
                                {["Event", "Accomodation", "Schedule", "Register"].map((item) => (
                                    <span key={item} className="text-[#d1f903] font-bold text-[1.1vw] uppercase tracking-widest cursor-pointer hover:scale-110 transition-transform duration-300">
                                        {item}
                                    </span>
                                ))}
                            </motion.div>

                            {/* EXISTING LAYERS REVEALED AFTER VIDEO */}
                            ...
                            <motion.div className="absolute inset-0 z-30 pointer-events-none" style={{ perspective: "1200px" }}>
                                    ...
                                    <div className="relative w-full aspect-video rounded-[1.3vw] overflow-hidden bg-[#d1f904]" style={{ boxShadow: "-15px 30px 60px rgba(0,0,0,0.5)", border: "3px solid rgba(209, 249, 3, 0.3)" }}>
                                        ...
                                        <div className="absolute inset-0 flex items-center justify-center p-[12%]">
                                            <p className="text-[#1a1a1a] text-[1.4vw] leading-[1.6] font-serif text-center italic font-medium opacity-90">
                                                Oasis, the annual cultural extravaganza of Birla Institute of Technology and Science, Pilani...
                                            </p>
                                        </div>
                                    </div>
                                </motion.div>
                            ...
                            {/* Existing Scroll-Triggered Video Overlay */}
                            {renderVideo && (
                                <motion.div className="absolute inset-0 z-40" style={{ opacity: videoOpacity, y: videoY_smooth, scale: videoScale_smooth, filter: videoBlur_smooth }}>
                                    <video src="/videos/hero.mp4" loop playsInline muted autoPlay className="w-full h-full object-cover bg-black" />
                                </motion.div>
                            )}
                        </motion.div>
                    )}
